...
import { Field, InkButton, Panel, Select, TextInput } from "@/components/ledger";
...
export function NewExpensePanel({ groupId, members }: { groupId: string; members: Member[] }) {
  ...
  const payers = multiPayer
    ? members
        .map((m) => ({ userId: m.userId, amountCents: parseAmountToCents(payerAmounts[m.userId] ?? "") }))
              ...
              {members.map((m) => (
                <div key={m.userId} className="flex items-center gap-2">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setPayerAmounts((p) => ({ ...p, [m.userId]: e.target.value }))
                    }
                    inputMode="decimal"
                    placeholder="0.00"
                  ...
                  <div key={m.userId} className="flex items-center gap-2">
                    ...
                    {on && splitType !== "equal" ? (
                      <div className="flex items-center gap-1">
                        {splitType === "exact" ? <span className="num text-[11px] text-ink3">$</span> : null}
                        <input
                          value={values[m.userId] ?? ""}
                          onChange={(e) =>
                            setValues((p) => ({ ...p, [m.userId]: e.target.value }))
                          }
                          inputMode="decimal"
